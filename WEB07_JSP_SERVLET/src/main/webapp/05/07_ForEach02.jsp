...
<!-- 
varStatus : 현재 반복순서의 상태값을 가지고 있는 클래스
status : 현재 반복순서 객체 변수 
${status.count} : 1부터 시작한 반복의 현재 아이템
${status.index} : 0부터 시작한 반복의 현재 아이템
-->

<!-- 위의 pageContext에 저장된 movieList를 이용하여 반복실행문을 제작하되
영화제목으로 구성된 리스트를 만들어주세요 choose when otherwise를 이용하여
첫번쨰는 빨간글자 나머지는 검은색으로 표시 (사용할 조건문 : test=$"{status.first}"-->
<ul>
	<c:forEach var="movie" items="${movieList}" varStatus="status">
		<c:choose>
			<c:when test="${status.first}"> <!-- 목록의 첫번째만 빨간색으로 -->
...
<!-- 
${statuse.first} : 현재 루프가 처음이면 true 리턴 
${statuse.last} : 현재 루프가 마지막이라면 true 리턴 -->
