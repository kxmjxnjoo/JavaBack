...
--FLOOR(숫자) - 값보다 작은 적수 중 가장 큰수[실수를 무조건 버림(음수일 경우는 제외)]
...
select greatest(23,65,84,45,12,89) from dual;
select floor(123.1234) from dual;
--특정 테이블을 대상으로 하면서 필드값이 함수의 입력값이 아니라면 
--from뒤에 dual사용하여 함수의 결과를 볼 수 있음 




--문자 관련함수
--ascii(문자) - goekd answkdml dktmzl zhemrkqt
...
--DATE_ADD(날짜, INTERVAL 기준값 더할숫자) - 날짜에서 기준값 만큼 더함
...
--DATE_SUB(날짜, INTERVAL 기준값, 뺄숫자) - 날짜에서 기준 값 만큼 뺸다
...
--%c 월(0~ 12)						--%D 일(1st ~ 2njd, 3rd...)
--%d 일(01~31)						--%e 일(0 ~ 31)
--%f Microseconds (00000 to 99999)
--%H 시간(00 ~ 23)					--%h 시간( 00 ~ 12)
--%I 시간(00 ~ 12)					--%i 분(00 ~ 59)
--%j Day of ter year(001 to 365) 
--%k 시간 (0 ~ 23) 					--%l 시간(1 ~ 12)
