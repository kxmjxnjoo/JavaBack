...
public class Paging {
    ...
    // [prev] 11 12 13 14  - next 없음 , 게시물 갯수가 121~129 개 인경우
    ...
    private void paging() {
    	...
    	// 총 게시물 수(totalCount)를 한화면에 표시할 게시물 수(displayRow)로 나눠서   
    	// "총 페이지 갯수"를 계산
    	// 108 / 10  ->  10.8  ->   11.0  ->  11
    	// 75 / 10  -> 7.5   ->  8
    	
    	if( totalPage < endPage ) { // 총페이지가 화면에 표시할 끝페이지보다 작다면
    		endPage = totalPage; // endPage 를 총페이지수로 대체
