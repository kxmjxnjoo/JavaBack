...
public class CalendarClass01 {
	...
	public static void main(String[] args) {
		...
		// 싱글톤 방식의 클래스
		// 생성자가 private으로 접근 차단됨(외부에서 접근 금지
		// 외부에서 new Calendar() 사용안됨
		// 유일하게 실행된 new Calendar()는 클래스 내부에서 실행되어
		// 클래스 내부에 단 하나만 존재하게끔 생성된 레퍼런스 변수에 주소를 저장
		// 그리고 그 레퍼런스 변수가 갖고 있는 주소를 리턴받는 public static 형태의
		// getInstance 메소드로 인스턴스의 주소를 리턴받아 씀
		// 이후 다시 getInstance()메소드가 실행되도 현재 Heap에 이미 있는 인스턴스의 주소가 리턴됨
		Calendar tomorrow = Calendar.getInstance();

		// 캘린더 클래스에는 스태틱 파이널(static final) 변수들이 다수 있음
		System.out.println(Calendar.YEAR);
		// get 메소드는 스태틱 파이널 변수에 해당하는 날짜 요소를 얻어주는 메소드
		System.out.println("년도: " + today.get(Calendar.YEAR));
		System.out.println("년도: " + today.get(1));
		
		int i = today.get(Calendar.MONTH);
		System.out.println("월: (0~11) : " + i + "월");
		// MONTH로 얻어낸 월의 값은 1월이 0 2월이 1.. 로 얻어짐
