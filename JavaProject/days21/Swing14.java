...
class Calendars extends JFrame implements ActionListener{
	static int year = 0;
	static int month = 0;
	static JTextField[] jt = new JTextField[42]; //42개짜리 레퍼런스 변수일 뿐이다
	JTextField y;
	JTextField m;
	...
	private void drawCalendar() {
		...
		//TextField가 다 비워져 있는 상태에서 시작	
		//1일자의 요일(1,2,3,4...)번째(START_WEEK-1)의 TextField부터 날짜 표시
		//이전 칸들은 비워둔채로 시작
		//각 TextField에 sDay의 값들을 표시 -> 날짜만 표시, 한번 반복마다 날짜는 1일씩 늘어남
		// eDay보다 sDay가 작거나 같을 때 까지 반복
