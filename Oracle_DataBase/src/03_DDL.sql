...
CREATE table booklist(
	...
	makeyear number(4) default 2021,   -- number(4) 는 4바이트 숫자가 아니라   4자리 숫자
	inprice number(6) default 0,
	rentprice number(6) default 0 ,     -- 마지막 필드를 쓰고 더 쓸내용이 없으면   컴마(,)를 쓰지 않습니다
...
--테이블의 이름은 객체를 의미할 수 있는 적잘한 이름을 사용합니다(자바 변수 이름 규칙과 거의 동일)
...
--컬럼과 컬럼의 구분은 콤마로 하되, 작성이 모두 종료되는 마지막 컬럼 또는 제약사항의 내용 뒤에는 콤마를 찍지 않습니다
...
create table memberlist(
	...
	joinDate date default sysdate,  -- 오늘날짜를 표시하는 오라클의 키워드
...
-- rent_date  와 idx 를 조합하여 기본키(rent_pk)를 생성합니다. 두개의 필드가 조합되어 기본키로 지정될
--       수 있습니다.   이를 슈퍼키라고 합니다.
